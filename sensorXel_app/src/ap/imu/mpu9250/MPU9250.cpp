...
#define MPU9250_SPI_CH      _DEF_SPI1
#define MPU9250_ADDRESS     0x68
#define MPU_CALI_COUNT      256
...
bool cMPU9250::begin()
{
  ...
  data = imu_spi_read(MPU9250_SPIx_ADDR, 0x75);
  data = imu_spi_read(MPU9250_SPIx_ADDR, 0x75);

  if((data == 0x73)||(data == 0x71)) // 0x73 for OLD Chips
  {
    bConnected = true;
    init();
    gyro_init();
...
void cMPU9250::init( void )
{
  ...
  //MPU9250 Set SPI Mode
	state = imu_spi_read(MPU9250_SPIx_ADDR, MPU9250_USER_CTRL);
	delay(1);
	imu_spi_write(MPU9250_SPIx_ADDR, MPU9250_USER_CTRL, state | MPU9250_I2C_IF_DIS);
	delay(1);
	state = imu_spi_read(MPU9250_SPIx_ADDR, MPU9250_USER_CTRL);
	delay(1);
	imu_spi_write(MPU9250_SPIx_ADDR, MPU9250_USER_CTRL, state | MPU9250_I2C_MST_EN);
	delay(1);
	//////////////////////////////////////////////////////////////////////////
	//AK8963 Setup
	//reset AK8963
...
void cMPU9250::gyro_get_adc( void )
{
  ...
  if( bConnected == true )
  {
    imu_spi_reads( MPU9250_SPIx_ADDR, MPU9250_GYRO_XOUT_H, 6, rawADC );
...
void cMPU9250::acc_get_adc( void )
{
  ...
  if( bConnected == true )
  {
    imu_spi_reads( MPU9250_SPIx_ADDR, MPU9250_ACCEL_XOUT_H, 6, rawADC );
...
void cMPU9250::mag_get_adc( void )
{
  ...
  if( bConnected == true )
  {
  	imu_spi_reads(MPU9250_SPIx_ADDR, MPU9250_EXT_SENS_DATA_00, 8, data);
